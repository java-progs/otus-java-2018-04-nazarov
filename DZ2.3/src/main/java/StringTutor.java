...
public class StringTutor  {
    ...
    public boolean checkGreeting(String greeting) {
        ...
        greeting = greeting.substring(prefix.length(),greeting.length()-1).trim();

        if (greeting.indexOf(" ") == -1) {
            return false;
        }

        String name = greeting.substring(0, greeting.indexOf(" "));
        if (name.length() < 3) {
            return false;
        }
        if (!name.substring(0, 1).equals(name.toUpperCase().substring(0, 1))) {
            return false;
        }

        greeting = greeting.substring(greeting.indexOf(" ") + 1, greeting.length()).trim();
        if (greeting.indexOf(" ") > 0) {
            return false;
        }

        String surname = greeting;
        if (surname.length() < 3) {
            return false;
        }
        if (!surname.substring(0, 1).equals(surname.toUpperCase().substring(0, 1))) {
            return false;
        }
